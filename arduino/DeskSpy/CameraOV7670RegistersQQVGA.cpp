...
// First few lines are padding.
// For some reason increasing vstart will not remove the first line, and causes synchronization problems.
// It is easier read all lines from the beginning and ignore the padding lines in the code.
const uint8_t CameraOV7670Registers::QQVGA_VERTICAL_PADDING = 2;
const uint16_t vstart = 0;
const uint16_t vstop = 120 + CameraOV7670Registers::QQVGA_VERTICAL_PADDING;// 120 + 2 pixel (4 bytes) for padding.
